...
import CreateTemplate from '@/components/layout/CampaignPages/Templates/CreateTemplate'
import TemplateTab from '@/components/layout/CampaignPages/Templates/TemplateTab'
...
export const EmptyState = ({ title, description, actionText, actionIcon, genericImage }: EmptyStateProps) => {
    ...
    return (
        <div className="flex flex-col items-center justify-center py-12 space-y-3 text-center">
                <Image src={type ==='campaign' ? '/jet.png' : type==='template'? '/Layout.png' :genericImage as string } width={150} height={150} alt='jet'/>
            <div className="mx-auto max-w-md">
                <h3 className="text-lg font-semibold text-gray-900 mb-2">{title}</h3>
                <p className="text-gray-600 mb-6">{description}</p>
                {actionText && <PrimaryButton isLink href={createAction} className='flex items-center gap-2'>
                    {actionIcon}
                    {actionText}
                </PrimaryButton>}
            </div>
