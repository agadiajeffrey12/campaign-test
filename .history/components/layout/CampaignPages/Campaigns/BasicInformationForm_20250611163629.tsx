...
const BasicInformationForm = (
  ...
  return (
    <div className='md:w-[525px] w-[350px] min-h-[660px] h-fit gap-[32px] flex flex-col items-center'>
        <h2 className='text-[14px] text-secondary-text font-[400]'>Fill out key info to personalize and direct your campaign.</h2>
          ...
          />
           <Input
            ...
            error={errors?.name}
          />
          <PrimaryButton fullWidth onClick={() => handleStepChange && handleStepChange(2)}>
            Save and Continue
          </PrimaryButton>
          <TertiaryButton fullWidth onClick={() => handleStepChange && handleStepChange(1)}>
            Cancel
            </TertiaryButton>
    </div>
  )
