...
const ConfirmAndSend = (
  ...
  const handleTextFormat = (command: string, value: string | null = null) => {
    if (typeof document !== 'undefined') {
      ...
      if (handleInputChange) {
        handleInputChange('message')({
          target: { value: content } as EventTarget & HTMLInputElement,
          // Add minimal required properties for a SyntheticEvent
          preventDefault: () => {},
          stopPropagation: () => {},
          nativeEvent: {} as any,
          bubbles: false,
...
const handleEmailBodyChange = (e: React.FormEvent<HTMLDivElement>) => {
...
const handleCampaignTimingChange = (value: string) => {
    setCampaignTiming(value);
    if (handleInputChange) {
        const timeData: TimeData = value === 'sendNow' ? {
            date: new Date(),
        ...
        handleInputChange('time')({
            target: { value: JSON.stringify(timeData) } as EventTarget & HTMLInputElement,
            preventDefault: () => {},
            stopPropagation: () => {},
            nativeEvent: {} as any,
            bubbles: false,
...
interface ScheduleDateChangeEvent {
    target: { value: { date: Date; time: string } };
}
...
const handleScheduleDateChange = (date: string) => {
    ...
    if (campaignTiming === 'scheduleLater' && handleInputChange) {
        handleInputChange('time')({
            target: { value: JSON.stringify({ date: new Date(date), time: scheduleTime }) } as EventTarget & HTMLInputElement,
            preventDefault: () => {},
            stopPropagation: () => {},
            nativeEvent: {} as any,
            bubbles: false,
...
interface ScheduleTimeChangeEvent {
    target: { value: string };
}
...
const handleScheduleTimeChange = (time: string) => {
    ...
    if (campaignTiming === 'scheduleLater' && handleInputChange) {
        handleInputChange('time')({
            target: { value: JSON.stringify({ date: new Date(scheduleDate), time }) } as EventTarget & HTMLInputElement,
            preventDefault: () => {},
            stopPropagation: () => {},
            nativeEvent: {} as any,
            bubbles: false,
  ...
  const handleFollowUpToggle = () => {
    ...
    if (handleInputChange) {
      handleInputChange('canFollowUpUsers')({
        target: { value: String(newValue) } as EventTarget & HTMLInputElement,
        // Add minimal required properties for a SyntheticEvent
        preventDefault: () => {},
        stopPropagation: () => {},
        nativeEvent: {} as any,
...
interface FollowUpSubjectChangeEvent {
    target: { value: string };
}

const handleFollowUpSubjectChange = (e: FollowUpSubjectChangeEvent) => {
    const value = e.target.value;
    setFollowUpSubject(value);
    if (handleInputChange) {
        handleInputChange('newSubject')({
            target: { value } as EventTarget & HTMLInputElement,
            preventDefault: () => {},
            stopPropagation: () => {},
            nativeEvent: {} as any,
...
interface GetPlainTextLength {
    (html: string): number;
}

const getPlainTextLength: GetPlainTextLength = (html) => {
    if (typeof document !== 'undefined') {
  ...
  return (
    <div className='md:w-[830px] w-full min-h-[1700px] flex flex-col space-y-[32px]'>
